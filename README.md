...
6. 「シェア用 URL をコピー」や X / Facebook ボタンで、入力状態を含む URL を共有できます（スマホはアプリ／OS共有シート、PC はブラウザで開きます）。
...
- スマホの X / Facebook 共有を改善（iOS=アプリ直接 / Android=OS共有シート経由でアプリに文章付きで渡す / PC=ブラウザ）。
